...
class WorkflowProcedureCycleStageDurationWizard(models.TransientModel):
    _name = "workflow.procedure.cycle.stage.duration.wizard"
    _description = "Configure Procedure Cycle Stage Duration"

    workflow_procedure_cycle_stage_id = fields.Many2one('workflow.procedure.cycle.stage',
    ...
    def create_procedure_cycle_stage_duration_from_wizard(self):
        temporal_units = ['minute', 'hour', 'day', 'week', 'month', 'quarter', 'semester', 'year']
        time_unit = self.time_unit
        min_duration = self.min_duration
        max_duration = self.max_duration
        data_list = [{
            'workflow_procedure_cycle_stage_id': self.workflow_procedure_cycle_stage_id.id,
            'time_unit': time_unit,
        ...
        self.env['workflow.procedure.cycle.stage.duration'].create(data_list)